...
public class Parser
{
	...
	public OptionalLong parseAsUnsignedLong()
	{
		...
		{
			EmbedUtil.sendError(event, "Invalid ID entered.");
	...
	public Optional<Boolean> parseAsBoolean()
	{
		...
		{
			EmbedUtil.sendError(event, "Invalid true / false entered.");
	...
	public Optional<Guild> parseAsGuild()
	{
		...
		return event.getValorous().getJDA().getGuilds().stream().filter(guild -> guild.getId().equals(arg)).findFirst();
	}
	...
	public OptionalInt parseAsUnsignedInt()
	{
		try
		{
			...
			if(value.getAsInt() == 0)
			{
				EmbedUtil.sendError(event, "Enter a whole number greater than 0, eg: 1");
	...
	private void parseAsMentionable(Consumer<IMentionable> consumer, Message.MentionType type)
	{
		...
		if(!message.getMentions().isEmpty()) //Direct mention
		{
			...
			if(mentions.isEmpty())
			{
				EmbedUtil.sendError(event, "No " + typeName.toLowerCase() + "s with name " + arg + " found.");
				return;
			}
		...
		if(idMatcher.matches()) //ID
		{
			...
			if(type == Message.MentionType.USER)
			{
				...
				{
					jda.retrieveUserById(mentionableId).queue(consumer, failure -> EmbedUtil.sendError(event, "No " + typeName.toLowerCase() + "s with name " + arg + " found."));
				}
				return;
			}
			else if(type == Message.MentionType.CHANNEL)
			{
				...
				{
					EmbedUtil.sendError(event, "No " + typeName.toLowerCase() + "s with name " + arg + " found or i dont have permission to see it.");
				}
				return;
			}
